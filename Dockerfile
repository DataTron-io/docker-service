...
RUN yum -y update && yum install -y yum-utils && yum groupinstall -y development \
    ...
    && python3.6 -m pip install -U datatron.common.discovery --index-url=http://${DEVPI_SERVER_ACCESS}@${DEVPI_SERVER_IP}:${DEVPI_SERVER_PORT}/datatron/pypi/+simple --trusted-host ${DEVPI_SERVER_IP} \
